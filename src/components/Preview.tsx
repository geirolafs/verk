  ...
  useEffect(() => {
    ...
    async function load() {
      const [commits, branches, tree] = await Promise.all([
        git.getRecentCommits(project!.path, 5),
        git.getBranches(project!.path),
        getTree(project!.path),
      ]);
...
async function getTree(path: string): Promise<string[]> {
  try {
    const proc = Bun.spawn(["eza", "--tree", "--level=2", "--icons=never", path], {
      stdout: "pipe",
      stderr: "ignore",
    });
    ...
    try {
      const proc = Bun.spawn(["ls", path], { stdout: "pipe", stderr: "ignore" });
      const text = await new Response(proc.stdout).text();
      return text.split("\n").filter(Boolean);
    } catch {
      return [];
